...
let ranking = JSON.parse(localStorage.getItem("capyRanking")) || [];
...
// Inicializar ranking en la tabla al cargar la página
mostrarRanking();
...
window.addEventListener("resize", () => {
  canvas.width = window.innerWidth;
  canvas.height = window.innerHeight;
  moon.x = canvas.width / 2 - 60; // ajustar luna con resize
  cameraY = Math.min(cameraY, FULL_BG_HEIGHT - canvas.height);
});

// Mostrar ranking inicial al cargar
mostrarRanking();
